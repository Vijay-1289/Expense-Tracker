...
const Index = () => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from("expenses").select("*").eq('user_id', user.id).order("date", {
        ascending: false
      });
      if (error) {
  ...
  useEffect(() => {
    ...
    fetchExpenses();
    fetchBudget();
    const expensesSubscription = supabase.channel('expenses_channel').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'expenses',
  ...
  if (!user) {
    return <div className="min-h-screen bg-gradient-to-br from-purple-100 via-pink-100 to-rose-100 flex items-center justify-center p-4">
        <div className="text-center space-y-6 max-w-md w-full bg-white/80 backdrop-blur-lg rounded-2xl p-8 shadow-xl animate-fade-in">
          ...
          <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">Expense Tracker</h1>
          <p className="text-muted-foreground">Track your spending with style 💖</p>
          <Button onClick={handleLogin} disabled={isLoading} className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 transform transition-all duration-200 hover:scale-105">
            {isLoading ? "Signing in..." : "Sign in with Google ✨"}
          </Button>
        </div>
      </div>;
  }
  return <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 animate-fade-in">
          ...
          <div className="flex flex-wrap gap-2 md:gap-4">
            <SetBudgetDialog />
            <AddExpenseDialog />
            <Button variant="outline" onClick={() => supabase.auth.signOut()} className="hover:scale-105 transition-transform duration-200">
              Sign Out 👋
            </Button>
          </div>
        </div>

        {showBudgetAlert && budget && <Alert variant="destructive" className="animate-fade-in bg-red-50 border-red-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          ...
          <div className="rounded-2xl border bg-white/80 backdrop-blur-lg shadow-lg p-6 transform transition-all duration-200 hover:scale-105 animate-fade-in" style={{
          ...
          animationDelay: '200ms'
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {budget && <Button variant="destructive" size="sm" onClick={handleDeleteBudget} className="flex items-center gap-1">
        ...
        <div className={`h-[${isMobile ? '300px' : '400px'}] bg-white/80 backdrop-blur-lg rounded-2xl p-4 shadow-lg animate-fade-in`} style={{
        animationDelay: '300ms'
      }}>
          <ExpenseChart data={expenses.map(expense => ({
        ...
        animationDelay: '400ms'
          ...
          <div className="grid gap-4">
            {expenses.map((expense, index) => <ExpenseCard key={expense.id} title={expense.title} amount={expense.amount} category={expense.category} date={new Date(expense.date).toLocaleDateString('en-IN')} className="animate-fade-in bg-white/80 backdrop-blur-lg transform transition-all duration-200 hover:scale-[1.02]" style={{
